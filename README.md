...
- Schedules batches across a pool of Ray actors using a work-stealing coordinator. Retries failures with exponential backoff, re-queueing a failed multi-task batch as two halves so a poisoned task is bisected to a singleton in ~log2(batch) failures instead of condemning its batchmates, replaces workers that hang or crash (force-killing the old actor so its GPUs are actually freed, then promoting a pre-loaded standby in O(1) when `--standby N` is set), enforces the retry budget on hangs as well as exceptions, and isn't blocked by the deferred retry queue.
...
**Standby worker pool: replacement in O(1) (standby pool).** Replacement used to block the single-threaded scheduling loop for up to `WORKER_INIT_TIMEOUT_S` × 3 attempts while the replacement loaded its model - minutes at 70B scale, during which completed refs from healthy workers sat unharvested. `--standby N` (default 0 = the old behavior, exactly) creates N extra workers from the same factory at startup; they load the model inside the same init barrier as the primaries and then idle. When a worker is replaced (hang eviction or failed health check), the coordinator force-kills the bad actor, promotes a standby into the slot with a handle swap - no construction, no `ray.get`, no model load - and submits a replacement standby's construction *asynchronously*: it holds the `ObjectRef` of a readiness ping (`health_check.remote()`) and polls it with `ray.wait(timeout=0)` from the main loop, so the refill joins the pool on a later iteration without ever blocking the scheduler. Refill failures retry up to 3 times, then the pool permanently runs one smaller. If a replacement finds the pool empty (replacements outpaced refills), it falls back to the blocking path and logs that it did. The promoted worker receives pending work through the existing dispatch path, so the one-batch-in-flight-per-slot invariant is untouched. **GPU-memory cost:** each standby holds a full model resident and claims the same Ray resources as a regular worker - budget memory and scheduling capacity for `--workers + --standby` actors (with `--hf-device` the fractional claims are sized for `--workers` only, so standbys may not schedule on an already-full single GPU). The summary reports a `standby` block: configured size, promotions, refills started/completed/failed, empty-pool fallbacks, and the ready-pool size over time (`pool_timeline`, sampled on every change); the bench's `standby` gauge now reads the live ready-pool size.

**Per-task retry via batch splitting (split-retry).** Whole-batch retry re-ran B-1 healthy generations to recover one failure, and a *deterministically* bad task condemned all of its batchmates. Now, on any batch failure with retries remaining - transient exception, deterministic exception, or hang eviction - a multi-task batch re-queues as two midpoint halves (`batch[:mid]`, `batch[mid:]`; task order preserved, no RNG), each inheriting the parent's `retry_count + 1`; a singleton retries whole, as before. Repeated failures therefore binary-search the offender: a poisoned task is isolated to a singleton in ~log2(batch_size) failures while every half that excludes it succeeds. The budget is per task and splitting never resets it - halves inherit uniformly and batches are never merged, so all tasks in a batch share one count and a batch's effective `retry_count` is exactly the max over its tasks; each task terminates within `max_retries`, recorded exactly once. Deterministic failures participate in splitting (bisection is the point), but a deterministic *singleton* is terminal immediately: re-running it verbatim cannot change the outcome. Backoff (full jitter) is drawn independently per enqueued half so halves decorrelate; hang requeues still go straight to `pending` without backoff, as before. The summary reports `retry_splits`, and the `retried` count now increments once per enqueued half. **Fault injection interaction:** the decider's `batch_key` is the tuple of the batch's task_ids, so halves are *new* keys with fresh attempt counters - expected and documented (see `fault_injection.FailureDeciderImpl`): a given `--seed` produces a different failure pattern than a pre-split-retry run, but the task-level outcome sequence remains fully reproducible across runs at the same seed (pinned by `tests/test_fault_injection.py::TestFailureDeciderImpl::test_task_level_outcomes_reproducible_across_runs`).
...
Failure injection goes through a single shared `FailureDecider` actor that all fault-injecting workers query. The decider tracks per-batch attempt counts globally, so a decision is a function of `(seed, batch_content, attempt_number)` and not of which worker happened to receive the batch. With split-retry, halves are new batch keys with fresh attempt counters, so the key stream depends on which batches fail; the reproducibility guarantee is stated (and tested) at the task level: same seed, same per-task outcomes, run after run.
...
- **Split-retry is bisection, not attribution; true per-task retry is unbuilt.** A failed batch re-queues as halves (split-retry), isolating one bad task in ~log2(batch) failures - but recovering it in a 64-batch still re-executes ~63 healthy task-runs across the shrinking retries, because the coordinator learns which task failed only by re-running subsets, and the halved retry batches under-drive vLLM's continuous batching on the retry path. Per-task failure attribution from the worker (report exactly which tasks failed, re-queue only those) would remove both costs; that piece isn't built. Note also that splitting changes which FailureDecider keys exist, so injected terminal-failure counts at a given `--seed` are not comparable between pre- and post-split-retry harnesses (each is individually reproducible).
...
- **Worker replacement is O(1) only while the standby pool holds a ready worker.** `--standby N` (standby pool) makes replacement a handle swap with an asynchronous background refill, but it is opt-in and defaults to 0, where every replacement still blocks on a model load. Even with a pool: a failure burst larger than N drains it faster than refills complete and the excess replacements fall back to the blocking path (counted as `fallbacks` in the summary); a refill that fails 3 times permanently shrinks the pool for the rest of the run; and each standby costs a full resident model plus a regular worker's Ray resource claim, which on a single GPU with fractional HF claims (sized for `--workers` only) can mean standbys never schedule at all. Refills restock the pool, not slots: a slot that dies permanently (empty pool *and* the blocking fallback exhausting its 3 attempts) is never revived by a refill that completes later - the ready standby waits for the next slot failure instead, and the run continues at n-1 slots exactly as before the standby pool. Sizing N is a memory-for-latency trade the operator makes, not something the harness tunes.
