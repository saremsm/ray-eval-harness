...
- Schedules batches across a pool of Ray actors using a work-stealing coordinator. Retries transient failures with exponential backoff, replaces workers that hang or crash (force-killing the old actor so its GPUs are actually freed, then promoting a pre-loaded standby in O(1) when `--standby N` is set), enforces the retry budget on hangs as well as exceptions, and isn't blocked by the deferred retry queue.
